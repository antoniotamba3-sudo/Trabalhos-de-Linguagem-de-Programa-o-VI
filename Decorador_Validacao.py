def validar_tipos(*expected_types):
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if len(args) != len(expected_types): raise TypeError(f"Esperava {len(expected_types)}argumentos posicionais, mas recebeu {len(args)}")

        for arg, expected_type in zip(args, expected_types):
            if not isinstance(arg, expected_type): raise TypeError(f"Argumento '{arg}' deve ser  do tipo {expected_type.__name__}, mas é {type(arg).__name__}.")
